...
namespace Nav.Common.VSPackages.CrmDeveloperHelper.Controllers
{
    ...
    public class ExportXmlController
    {
        ...
        public async Task ExecuteConvertingFetchXmlToQueryExpression(string fetchXml, ConnectionData connectionData, CommonConfiguration commonConfig)
        {
            string operation = string.Format(Properties.OperationNames.DifferenceSavedQueryFormat1, connectionData?.Name);
            ...
            try
            {
                await ConvertingFetchXmlToQueryExpression(fetchXml, connectionData, commonConfig);
            }
            catch (Exception ex)
            {
                this._iWriteToOutput.WriteErrorToOutput(connectionData, ex);
        ...
        private async Task ConvertingFetchXmlToQueryExpression(string fetchXml, ConnectionData connectionData, CommonConfiguration commonConfig)
        {
            if (connectionData == null)
            {
                this._iWriteToOutput.WriteToOutput(connectionData, Properties.OutputStrings.NoCurrentCRMConnection);
            ...
            var response = await service.ExecuteAsync<FetchXmlToQueryExpressionResponse>(new FetchXmlToQueryExpressionRequest()
            {
                FetchXml = fetchXml,
            });
            ...
            using (var writer = new StringWriter(codeCSharp))
            {
                ...
                codeGenerator.WriteCSharpQueryExpression(response.Query);
            }
            ...
            System.Threading.Thread worker = new System.Threading.Thread(() =>
            {
                try
                {
                    Clipboard.SetText(codeCSharp.ToString());
                }
                catch (Exception ex)
                {
