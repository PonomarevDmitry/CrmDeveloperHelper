...
{
    internal sealed class FolderWebResourceUpdateContentPublishGroupConnectionCommand : IServiceProviderOwner
    {
        private readonly Package _package;

        public IServiceProvider ServiceProvider => this._package;

        private const int _baseIdStart = PackageIds.FolderWebResourceUpdateContentPublishGroupConnectionCommandId;

        private FolderWebResourceUpdateContentPublishGroupConnectionCommand(Package package)
        {
        ...
        {
            Instance = new FolderWebResourceUpdateContentPublishGroupConnectionCommand(package);
        }
                ...
                if (sender is OleMenuCommand menuCommand)
                {
                    ...
                    if (0 <= index && index < list.Count)
                    {
                        ...
                        if (!connectionData.IsReadOnly)
                        {
                            ...
                            CommonHandlers.ActionBeforeQueryStatusSolutionExplorerWebResourceTextRecursive(this, menuCommand);
                        }
                    }
                }
            }
