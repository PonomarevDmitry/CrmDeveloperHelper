...
{
    internal sealed class FileXmlSetXsdSchemaCommand : IServiceProviderOwner
    {
        private readonly Package _package;

        public IServiceProvider ServiceProvider => _package;

        private const int _baseIdStart = PackageIds.FileXmlSetXsdSchemaCommandId;

        private FileXmlSetXsdSchemaCommand(Package package)
        {
        ...
        {
            Instance = new FileXmlSetXsdSchemaCommand(package);
        }
            ...
            try
            {
                if (sender is OleMenuCommand menuCommand)
                {
                    ...
                    if (0 <= index && index < CommonExportXsdSchemasCommand.ListXsdSchemas.Count)
                    {
                        ...
                        CommonHandlers.ActionBeforeQueryStatusSolutionExplorerXmlAny(this, menuCommand);
                    }
                }
            }
            catch (Exception ex)
