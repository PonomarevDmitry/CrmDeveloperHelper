...
{
    internal sealed class FileJavaScriptUpdateGlobalOptionSetAllFileCommand : IServiceProviderOwner
    {
        private readonly Package _package;

        public IServiceProvider ServiceProvider => this._package;

        private const int _baseIdStart = PackageIds.FileJavaScriptUpdateGlobalOptionSetAllFileCommandId;

        private FileJavaScriptUpdateGlobalOptionSetAllFileCommand(Package package)
        {
        ...
        {
            Instance = new FileJavaScriptUpdateGlobalOptionSetAllFileCommand(package);
        }
            ...
            try
            {
                if (sender is OleMenuCommand menuCommand)
                {
                    ...
                    if (0 <= index && index < connectionsList.Count)
                    {
                        ...
                        CommonHandlers.ActionBeforeQueryStatusSolutionExplorerJavaScriptSingle(this, menuCommand);
                    }
                }
            }
            catch (Exception ex)
