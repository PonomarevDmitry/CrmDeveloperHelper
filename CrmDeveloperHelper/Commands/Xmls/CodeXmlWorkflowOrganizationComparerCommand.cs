...
{
    internal sealed class CodeXmlWorkflowOrganizationComparerCommand : AbstractDynamicCommandConnectionPair
    {
        private CodeXmlWorkflowOrganizationComparerCommand(OleMenuCommandService commandService, int baseIdStart, string formatButtonName)
            : base(commandService, baseIdStart, formatButtonName)
        {
        }

        public static CodeXmlWorkflowOrganizationComparerCommand Instance { get; private set; }
        ...
        protected override void CommandAction(DTEHelper helper, Tuple<ConnectionData, ConnectionData> connectionDataPair)
        {
            ...
            if (selectedFiles.Count == 1)
            {
                ...
                if (ContentComparerHelper.TryParseXml(fileText, out var doc))
                {
                    ...
                    helper.HandleOpenWorkflowOrganizationComparerCommand(connectionDataPair.Item1, connectionDataPair.Item2, attribute?.Value);
                }
            }
        }
