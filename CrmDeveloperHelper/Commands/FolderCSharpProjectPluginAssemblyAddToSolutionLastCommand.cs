...
{
    internal sealed class FolderCSharpProjectPluginAssemblyAddToSolutionLastCommand : IServiceProviderOwner
    {
        private readonly Package _package;

        public IServiceProvider ServiceProvider => this._package;

        private const int _baseIdStart = PackageIds.FolderCSharpProjectPluginAssemblyAddToSolutionLastCommandId;

        private FolderCSharpProjectPluginAssemblyAddToSolutionLastCommand(Package package)
        {
        ...
        {
            Instance = new FolderCSharpProjectPluginAssemblyAddToSolutionLastCommand(package);
        }
            ...
            try
            {
                ...
                if (connectionConfig.CurrentConnectionData != null)
                {
                    ...
                    if (0 <= index && index < connectionConfig.CurrentConnectionData.LastSelectedSolutionsUniqueName.Count)
                    {
                        ...
                        if (list.Any())
                        {
                            helper.HandleAddingPluginAssemblyToSolutionByProjectCommand(null, solutionUniqueName, false, list.Distinct(StringComparer.InvariantCultureIgnoreCase).ToArray());
                        }
                    }
                }
            }
