...
namespace Nav.Common.VSPackages.CrmDeveloperHelper.Views
{
    public partial class WindowExportReport : WindowBase
    {
        ...
        private void LoadReports(IEnumerable<Report> results)
        {
            ...
            UpdateStatus(string.Format("{0} Reports loaded.", results.Count()));
        ...
        private Report GetSelectedEntity()
        {
            ...
            if (this.lstVwReports.SelectedItems.Count == 1
                && this.lstVwReports.SelectedItems[0] != null
                && this.lstVwReports.SelectedItems[0] is EntityViewItem
                )
            {
                result = (this.lstVwReports.SelectedItems[0] as EntityViewItem).Report;
            }
        ...
        private string CreateFile(string folder, string name, Guid id, string fieldTitle, string xmlContent)
        {
            ...
            if (!string.IsNullOrEmpty(xmlContent))
            {
                try
                {
                    ...
                    this._iWriteToOutput.WriteToOutput("{0} Report {1} {2} exported to {3}", connectionData.Name, name, fieldTitle, filePath);
                }
                catch (Exception ex)
                {
                    this._iWriteToOutput.WriteErrorToOutput(ex);
            ...
            {
                this._iWriteToOutput.WriteToOutput("Report {0} {1} is empty.", name, fieldTitle);
        ...
        private void mIExportReportDependentComponents_Click(object sender, RoutedEventArgs e)
        {
            var entity = GetSelectedEntity();
            ...
            ExecuteAction(entity.Id, entity.Name, entity.FileName, PerformCreatingFileWithDependentComponents);
        }

        private void mIExportReportRequiredComponents_Click(object sender, RoutedEventArgs e)
        {
            var entity = GetSelectedEntity();
            ...
            ExecuteAction(entity.Id, entity.Name, entity.FileName, PerformCreatingFileWithRequiredComponents);
        }

        private void mIExportReportDependenciesForDelete_Click(object sender, RoutedEventArgs e)
        {
            var entity = GetSelectedEntity();
            ...
            ExecuteAction(entity.Id, entity.Name, entity.FileName, PerformCreatingFileWithDependenciesForDelete);
        }

        private async Task PerformCreatingFileWithDependentComponents(string folder, Guid idReport, string name, string filename)
        {
            ...
            var coll = await dependencyRepository.GetDependentComponentsAsync((int)ComponentType.Report, idReport);

            string description = await descriptorHandler.GetDescriptionDependentAsync(coll);

            if (!string.IsNullOrEmpty(description))
            {
                string fileName = EntityFileNameFormatter.GetReportFileName(
                ...
                this._iWriteToOutput.WriteToOutput("Report {0} Dependent Components exported to {1}", name, filePath);

                this._iWriteToOutput.PerformAction(filePath, _commonConfig);
            }
            else
            {
                this._iWriteToOutput.WriteToOutput("Report {0} has no Dependent Components.", name);
                this._iWriteToOutput.ActivateOutputWindow();
            }
        }

        private async Task PerformCreatingFileWithRequiredComponents(string folder, Guid idReport, string name, string filename)
        {
            this._iWriteToOutput.WriteToOutput("Starting downloading {0}", name);
            ...
            var coll = await dependencyRepository.GetRequiredComponentsAsync((int)ComponentType.Report, idReport);
            ...
            if (!string.IsNullOrEmpty(description))
            {
                ...
                this._iWriteToOutput.WriteToOutput("Report {0} Required Components exported to {1}", name, filePath);

                this._iWriteToOutput.PerformAction(filePath, _commonConfig);
            }
            else
            {
                this._iWriteToOutput.WriteToOutput("Report {0} has no Required Components.", name);
                this._iWriteToOutput.ActivateOutputWindow();
            }
        }

        private async Task PerformCreatingFileWithDependenciesForDelete(string folder, Guid idReport, string name, string filename)
        {
            this._iWriteToOutput.WriteToOutput("Starting downloading {0}", name);
            ...
            var coll = await dependencyRepository.GetDependenciesForDeleteAsync((int)ComponentType.Report, idReport);
            ...
            if (!string.IsNullOrEmpty(description))
            {
                ...
                this._iWriteToOutput.WriteToOutput("Report {0} Dependencies For Delete exported to {1}", name, filePath);

                this._iWriteToOutput.PerformAction(filePath, _commonConfig);
            }
            else
            {
                this._iWriteToOutput.WriteToOutput("Report {0} has no Dependencies For Delete.", name);
                this._iWriteToOutput.ActivateOutputWindow();
            }
        }
            ...
            if (connectionData != null)
            {
                ...
                var backWorker = new Thread(() =>
                {
                    try
                    {
                        ...
                        contr.ExecuteAddingComponentesIntoSolution(connectionData, _commonConfig, solutionUniqueName, ComponentType.Report, new[] { entity.Id }, null, withSelect);
                    }
                    catch (Exception ex)
                    {
                        this._iWriteToOutput.WriteErrorToOutput(ex);
